...
export class AirlinesComponent implements OnInit {
  ...
  filteredAirlines = new Array<Airline>();
  ...
  constructor(public loadService: LoadDataService, private http: HttpClient, private airlineService: RegularUserService) { }
  ...
  //#endregion
  //#region 7 - Metode za filtriranje aviokompanija
  filterAirlines(){
    let filterParams = new Array<AbstractFilterParam>();
    if (this.getFilterFieldValue("airlineNameFilter")) {
      filterParams.push(this.addAirlineNameFilterParam());
    }
    if (this.getFilterFieldValue("airlineRatingFilter")) {
      filterParams.push(this.addAirlineRatingFilterParam());
    }

    this.filteredAirlines = this.airlineService.filterAirlines(this.airlines, filterParams);
  }
  ...
  addAirlineNameFilterParam(): ReturnType<any> {
    return new StringFilterParam("airlineNameFilter", this.getFilterFieldValue("airlineNameFilter"));
  }

  addAirlineRatingFilterParam(): ReturnType<any> {
    return new NumberFilterParam("airlineRatingFilter", +this.getFilterFieldValue("airlineRatingFilter"));
  }
  //#endregion
}
